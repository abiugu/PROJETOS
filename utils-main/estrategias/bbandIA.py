...
modelo_ml = joblib.load("xgboost_model.pkl")
...
async def startStrategy(symbol, send_message):
    ...
    # Preprocessar os dados para o modelo
    def preprocess_data(df):
        # Seleciona as mesmas features usadas durante o treinamento do modelo
        features = ['RSI', 'BBL', 'BBM', 'BBU', 'largura', 'VPT','EMA26']
        df_prepared = df[features].tail(1)  # Pega apenas o último dado
        return df_prepared

    # Fazer a previsão com base nos dados mais recentes
    dados_preprocessados = preprocess_data(df_candles)
    previsao = modelo_ml.predict(dados_preprocessados)

    # Integração da previsão com a lógica de negociação
    if previsao == 1:  # Sinal de compra (long)
        if not gr.posicao_max(symbol, posicao_max) and gr.posicoes_abertas(symbol)[0] != 'short' and not gr.ultima_ordem_aberta(symbol):
            try:
                ...
                message = f"******* ABRINDO LONG DE {amount} MOEDAS EM {symbol} BBANDS + IA! *******"
                logger.info(message)
    ...
    elif previsao == 0:  # Sinal de venda (short)
        if not gr.posicao_max(symbol, posicao_max) and gr.posicoes_abertas(symbol)[0] != 'long' and not gr.ultima_ordem_aberta(symbol):
            try:
                ...
                message = f"******* ABRINDO SHORT DE {amount} MOEDAS EM {symbol} BBANDS + IA! *******"
                logger.info(message)
                await send_message(message)
            except Exception as e:

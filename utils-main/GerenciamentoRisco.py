...
trailing_loss = 0  # Variável global
...
async def fecha_pnl(symbol, loss, target, send_message, timeframe):
    ...
    try:
        ...
        unidade = timeframe[-1]
        unidades = {'m': num_timeframe * 5 * 60, 'h': num_timeframe * 2 * 3600, 'd': num_timeframe * 0.5 * 86400}
        t_sleep = unidades.get(unidade, 60)  # Valor padrão de 60s caso a unidade seja inválida

        if percent:
            # Verificando condições para encerrar posição
            if percent <= loss:
                msg = f"❌ Posição encerrada por perda: {pnl}. Saldo atual: {balance}."
                encerrar_posicao(symbol=symbol)
                await send_message(msg)
                await asyncio.sleep(t_sleep)

            elif percent >= target:
                msg = f"✅ Posição encerrada por ganho: {pnl} em {symbol}. Saldo atual: {balance}. Parabéns!"
                encerrar_posicao(symbol=symbol)
                await send_message(msg)

            elif percent <= trailing_loss:
                msg = f"🔄 Posição encerrada por trailing stop com ganho: {pnl} em {symbol}. Saldo atual: {balance}. Proteção de lucro ativada!"
                encerrar_posicao(symbol=symbol)
                await send_message(msg)

            # Atualização do trailing_loss baseada nos percentuais de lucro
            ...
            for threshold, new_trailing in loss_thresholds:
                ...
                # Se nenhuma condição for atendida, trailing_loss é ajustado para o valor original do loss
                trailing_loss = loss

    except Exception as e:
        error_msg = f"⚠️ Erro ao verificar PnL ou encerrar posição: {e}"
